...
Form::Form( const std::string name, const std::string target, bool isSigned, const int signGrade, const int execGrade ) :
	_name(name),
	_target(target),
	_isSigned(isSigned),
...
std::ostream& operator<<( std::ostream& data, const Form& some) {
	...
	data << "Form's target: \"" << some.getTarget() << "\"" << std::endl;
